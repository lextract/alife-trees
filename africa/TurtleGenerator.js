...
class TurtleGenerator {
    ...
    rotate(positive) {
        if (positive)
            this.lastAngle = this.deltaAngle;
        else
            this.lastAngle = -this.deltaAngle;
        this.lastRotation.z += this.lastAngle;
        if (positive)
            this.head.applyAxisAngle(zUnitVector, this.deltaAngle);
        else
            this.head.applyAxisAngle(zUnitVector, -this.deltaAngle);
    }
    executeLang(lang, scene) {
    ...
    matrixRU(delta) {
        ...
        m.set(Math.cos(delta), Math.sin(delta), 0, -Math.sin(delta), Math.cos(delta), 0, 0, 0, 1);
        return m;
    }
    matrixRL(delta) {
        let m = new THREE.Matrix3();
        m.set(Math.cos(delta), 0, -Math.sin(delta), 0, 1, 0, Math.sin(delta), 0, Math.cos(delta));
        return m;
    }
    matrixRH(delta) {
        let m = new THREE.Matrix3();
        m.set(1, 0, 0, 0, Math.cos(delta), -Math.sin(delta), 0, Math.sin(delta), Math.cos(delta));
        return m;
    }
}
exports.TurtleGenerator = TurtleGenerator;
