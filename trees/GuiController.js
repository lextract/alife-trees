"use strict";
const LanguageGenerator = require('./LanguageGenerator');
...
function GuiController(appModule, stage) {
    appModule.controller('GuiController', ($scope) => {
        ...
        $scope.createTree = function () {
            let axiom = $scope.axiom;
            let rules = new Map();
            let prod1 = LanguageGenerator.extractRule($scope.prod1);
            if (prod1)
                rules.set(prod1.key, prod1.value);
            let prod2 = LanguageGenerator.extractRule($scope.prod2);
            if (prod2)
                rules.set(prod2.key, prod2.value);
            let prod3 = LanguageGenerator.extractRule($scope.prod3);
            if (prod3)
                rules.set(prod3.key, prod3.value);
            let prod4 = LanguageGenerator.extractRule($scope.prod4);
            if (prod4)
                rules.set(prod4.key, prod4.value);
            let prod5 = LanguageGenerator.extractRule($scope.prod5);
            if (prod5)
                rules.set(prod5.key, prod5.value);
            arboles.push(new Tree_1.Tree(new THREE.Vector3(parseFloat($scope.rootXcoord), parseFloat($scope.rootYcoord), parseFloat($scope.rootZcoord)), LanguageGenerator.grammar(axiom, rules), parseFloat($scope.deltaAngle), parseInt($scope.maxIterations)));
        };
